...
We said that NumPy arrays and functions give us performance. Let us see if that is the case and compare the performance of np.dot with both list vectors and array vectors with the performance of our own dot product code. To obtain reasonable timings we want to use large arrays. Instead of filling these arrays simply with zeros or ones we want to use more realistic data. We can do this by using the function `np.random.random(N)`, which produces an array of size N containing uniform random numbers between `0` and `1`:

    m = np.random.random(1000000)
    n = np.random.random(1000000)

We can now use IPython's `timeit` command to see how long it takes to perform the dot product of `m` and `n` using the NumPy function:

    timeit np.dot(m,n)
  
Note that `timeit` is an IPython [magic function](http://ipython.org/ipython-doc/dev/interactive/tutorial.html) not a Python command. Magic functions have built-in help, for example:

    timeit?

To time our own dot product on the same data we want to convert `m` and `n` into lists, this can be done as follows:

    mlist = m.tolist()
    nlist = n.tolist()
    timeit dot(mlist, nlist)

Finally we can also time how NumPy's dot product performs with these lists:

    timeit np.dot(mlist, nlist)

Typically the NumPy dot product with arrays is over two orders of magnitude faster than our naive list-based implementation.
...
and compare it with the average maximum cell count for people who weren't showing signs of infection on day 1:
    ...
    print np.mean(np.max(patients[ patients[:, 1] == 0 ], 1))
...
Compare this to our expression:
    ...
    np.mean(np.max(patients[ patients[:, 1] == 0], 1))
